...
public:
    ...
    virtual void getSolutions(const VectorQ& lastConfig,
        ...
        const akin::Link& baseLink = this->_robot->dof(this->_dofs[0]).joint().upstreamLink();
        ...
        for(int i=0; i<8; ++i)
        {
            ...
            for(int j=0; j<6; ++j)
            {
                if(!this->_robot->dof(this->_dofs[j]).withinLimits(testQ[j]))
                {
                    add_result = false;
                    break;
                }
...
protected:
    ...
    virtual bool _reconfigure() {
        ...
        L4 = fabs(this->_robot->dof(this->_dofs[3]).joint().childLink().
                respectToRef().translation()[2]);
        L5 = fabs(this->_robot->dof(this->_dofs[4]).joint().childLink().
                respectToRef().translation()[2]);
        L6 = fabs(this->_manip->withRespectTo(this->_robot->dof(this->_dofs[5]).joint().
                  childLink()).translation()[2]);

        hipRotation = Eigen::Isometry3d::Identity();
        hipRotation.rotate(akin::Rotation(90*akin::DEG, akin::Vec3(0,0,1)));
        
        waist =  this->_robot->dof(this->_dofs[0]).joint().baseTransform()
                  *this->_robot->dof(this->_dofs[1]).joint().baseTransform()
                  *this->_robot->dof(this->_dofs[2]).joint().baseTransform()
                  *hipRotation;
